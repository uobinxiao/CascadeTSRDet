...
def fast_rcnn_inference(
    ...
    result_per_image = [
        fast_rcnn_inference_single_image(
            boxes_per_image, scores_per_image, image_shape, score_thresh, nms_thresh, topk_per_image, num_classes
        )
...
def fast_rcnn_inference_single_image(
    ...
    if num_classes == 7:
        ...
        indices_4 = (labels == 4).nonzero().squeeze(1)
        indices_6 = (labels == 6).nonzero().squeeze(1)
        expanded_box_list = []
        expanded_score_list = []
        expanded_inds_list = []
    
        if indices_4.shape[0] != 0:
            expanded_boxes_2 = boxes[indices_4]
            expanded_scores_2 = scores[indices_4]
            expanded_inds_2 = filter_inds[indices_4]
            expanded_inds_2[:, 1] = 2
            expanded_box_list.append(expanded_boxes_2)
            expanded_score_list.append(expanded_scores_2)
            expanded_inds_list.append(expanded_inds_2)
        
        if indices_6.shape[0] != 0:
            expanded_boxes_5 = boxes[indices_6]
            expanded_scores_5 = scores[indices_6]
            expanded_inds_5 = filter_inds[indices_6]
            expanded_inds_5[:, 1] = 5
            expanded_box_list.append(expanded_boxes_5)
            expanded_score_list.append(expanded_scores_5)
            expanded_inds_list.append(expanded_inds_5)

        filter_inds[indices_6, 1] = 2
        boxes= torch.cat([boxes] + expanded_box_list, dim = 0)
        scores = torch.cat([scores] + expanded_score_list, dim = 0)
        filter_inds = torch.cat([filter_inds] + expanded_inds_list, dim = 0)
...
class MyFastRCNNOutputLayers(nn.Module):
    ...
    def __init__(
        ...
        *,
        ...
        #    cls_module.append(nn.Linear(input_size, input_size, False))
        #    cls_module.append(nn.LayerNorm(input_size))
    ...
    def forward(self, x, proposal_features = None):
        ...
        #scores = self.cls_score(proposal_features)
    ...
    def losses(self, predictions, proposals):
        ...
        if self.use_sigmoid_ce:
            ...
            #loss_cls = F.cross_entropy(scores, gt_classes, reduction="mean", label_smoothing = 0.1)

        #src_logits = scores
        #labels = F.one_hot(gt_classes, num_classes=scores.shape[1])
