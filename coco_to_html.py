...
def get_args():
    ...
    parser.add_argument('--image_dir', help="Directory for input images")
    parser.add_argument('--words_dir', help="Directory for input words")
    parser.add_argument('--json_path', help="Directory for input words")
    parser.add_argument('--out_dir', help="Output directory")
    parser.add_argument('--mode', help="The processing to apply to the input image and tokens", choices=['detect', 'recognize', 'extract'])
    parser.add_argument('--objects', '-o', action='store_true', help='Output objects')
    parser.add_argument('--cells', '-l', action='store_true', help='Output cells list')
    parser.add_argument('--html', '-m', action='store_true', help='Output HTML')
    parser.add_argument('--csv', '-c', action='store_true', help='Output CSV')
    parser.add_argument('--verbose', '-v', action='store_true', help='Verbose output')
    parser.add_argument('--visualize', '-z', action='store_true', help='Visualize output')
...
class TableExtractionPipeline(object):
    def __init__(self, det_device=None, str_device=None,
        ...
        self.str_class_idx2name = {v+1:k for k, v in self.str_class_name2idx.items()}
    ...
    def recognize(self, img_file, coco, tokens=None, out_objects=False, out_cells=False,
                  out_html=False, out_csv=False):
        out_formats = {}

        all_images = coco.loadImgs(coco.getImgIds())
        # Filter images based on name
        filtered_images = [img for img in all_images if img['file_name'] == img_file]

        if len(filtered_images) == 0:
            return

        # Get image IDs for the filtered images
        image_ids = [img['id'] for img in filtered_images]

        # Retrieve the annotations for the image IDs
        ...
        for anno in annotations:
            bbox = anno["bbox"]
            box_list.append([bbox[0], bbox[1], bbox[0] + bbox[2], bbox[1] + bbox[3]])
            class_list.append(anno["category_id"])

        output_dict = {}
        output_dict["pred_boxes"] = box_list
        output_dict["scores"] = [1] * len(class_list)
        output_dict["pred_classes"] = class_list

        image_size = (filtered_images[0]["width"], filtered_images[0]["height"])
...
def main(coco, img_files):
    ...
    for count, img_file in enumerate(tqdm(img_files)):
        ...
        if args.mode == 'recognize':
            extracted_table = pipe.recognize(img_file, coco, tokens, out_objects=args.objects, out_cells=args.csv, out_html=args.html, out_csv=args.csv)
...
if __name__ == "__main__":
    ...
    num_task = 64
    step = len(img_files) // num_task

    #num_files = len(img_files)
    for i in range(num_task):
        if i == num_task - 1:
            Process(target=main, args=(coco, img_files[step * i : ], )).start()
        else:
            Process(target=main, args=(coco, img_files[step * i : step * (i + 1)], )).start()
